      ...
      >
        ...
        <el-table-column>
          ...
          <template #default="scope">
            <el-popconfirm title="确定要删除吗？" @confirm="handleDelete(scope.$index, scope.row.oid)">
              <template #reference>
                <el-button size="small" type="danger">删除</el-button>
              </template>
            </el-popconfirm>
            <el-button size="small" type="primary" @click="getOrderDetail(scope.$index, scope.row.oid)">详情</el-button>
            <el-button size="small" type="success" @click="handleUpdate(scope.$index, scope.row.oid)">审批</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div style="margin-top: 40px; display: flex; justify-content: space-around; flex-grow: 1"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  setup() {
    ...
    const handleUpdate = (index, oid) => {
      axios
        .post('/admin/updateOrder?oid=' + oid)
        .then((res) => {
          router.go(0)
        })
        .catch((error) => {
