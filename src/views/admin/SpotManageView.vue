...
<el-table-column type="index" width="60" label="序号"  :index="indexMethod((currPage-1)*5 + 1)">
    ...
    <el-table-column prop="sname" label="景点名" width="120" />
    <el-table-column label="图片" width="100">
      <template #default="scope">
        <div v-if="scope.row.spic != null">
          <img
            :src="require(`@/assets/uploadfile/${scope.row.spic}`)"
            width="80"
          />
        </div>
      </template>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      axios
        .get("/admin/getSpotByPage?currPage=" + data.currPage)
        .then((res) => {
   ...
   //设置序号
    ...
    const handleDelete=(index,row) => {
      ...
      axios.get("/admin/deleteSpotByID?sid="+row.sid)
      .then(res=>{
        if(res.data==="ok"){
          alert("删除成功！");
        //********刷新页面********************************* */
          axios.get("/admin/getSpotByPage?currPage=" + data.currPage)
        .then((res) => {
          data.SpotList = res.data.SpotList;
          data.hasNext = res.data.hasNext;
        });
    ...
    const handMultiDelete=() => {
      axios.post("/admin/deleteSpotMulti",data.SpotMulti)
      .then(res =>{
        if(res.data==="ok"){
          alert("删除成功！");
        //********刷新页面********************************* */
    ...
    const handleEdit=(index,row)=>{
      ...
      window.sessionStorage.setItem('Spot',JSON.stringify(row))
      router.push("/admin/updateSpot?currPage="+data.currPage);
    }
    const login = () => {};
