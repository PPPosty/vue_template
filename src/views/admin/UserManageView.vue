...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      axios
        .get("/user/getUserByPage?currPage=" + data.currPage)
        .then((res) => {
   ...
   //设置序号
    ...
    const handleDelete=(index,row) => {
      ...
      axios.get("/user/deleteUserByID?uid="+row.uid)
      .then(res=>{
        if(res.data==="ok"){
          alert("删除成功！");
        //********刷新页面********************************* */
          axios.get("/user/getUserByPage?currPage=" + data.currPage)
        .then((res) => {
          data.UserList = res.data.UserList;
          data.hasNext = res.data.hasNext;
        });
    ...
    const handMultiDelete=() => {
      axios.post("/admin/deleteUserMulti",data.UserMulti)
      .then(res =>{
        if(res.data==="ok"){
          alert("删除成功！");
        //********刷新页面********************************* */
